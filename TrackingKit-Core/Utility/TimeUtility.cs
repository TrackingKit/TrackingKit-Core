...
namespace Tracking
{
    public static class TimeUtility
    {
        ...
        {
            double currentTime = TimeFactory.Now;
            Seconds.Add(new KeyValuePair<double, int>(currentTime, TimeFactory.Tick));
        ...
        private static void DisperseOlderData()
        {
            ...
            if (DEBUG)
            {
                ...
                LogFactory.Info("Dispersion Report:");
                LogFactory.Info($"- Original Value: {originalValue} records");
                LogFactory.Info($"- Records Removed: {recordsRemoved} records ({GetPercentage(recordsRemoved, originalValue)} of original storage)");
                LogFactory.Info($"- Records Retained: {Seconds.Count} records ({GetPercentage(Seconds.Count, originalValue)} of original storage)");
                LogFactory.Info($"- Total Records after Cleanup: {Seconds.Count} records");

                LogFactory.Info($"- Most recent 25%: {recent25Count} records ({GetPercentage(recent25Count, totalCount)} of current storage)");
                LogFactory.Info($"- Next 25%: {next25Count} records ({GetPercentage(next25Count, totalCount)} of current storage)");
                LogFactory.Info($"- Next 25% again: {next25AgainCount} records ({GetPercentage(next25AgainCount, totalCount)} of current storage)");
                LogFactory.Info($"- Oldest 25%: {oldest25Count} records ({GetPercentage(oldest25Count, totalCount)} of current storage)");
            }
        }
